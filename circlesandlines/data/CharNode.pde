class CharNode {
  ...
  CharNode(String character, float posx, float posy, float diameter, color c) {
    baseX = posx;
    baseY = posy;
    x = baseX;
    y = baseY;
    ...
    //if ((48<= dec&& dec<=57) || (97<=dec && dec<=122)) {
    //  spec = false;
    //  oacc = false;
    //  if (dec==97 || dec==101
    //// Special characters.
    //} else if ((33<=dec && dec<=47) || (58<=dec && dec<=96) || (123<=dec && dec<=137) || dec==139 || (145<=dec && dec<=153) || dec==155 || (161<=dec && dec<=180) || (182<=dec && dec<=191) || dec==215 || dec==247) {
    //  spec = false;
    //  oacc = true;
    //// Accented letters, or letters in other languages.
    //} else {
    //  spec = true;
    //  oacc = false;
    //}
...
void makeNodes(IntDict charFreqs) {
  ...
  for (int i=0; i < charFreqs.size(); i++) {
    ...
    nodes.put(keys[i], new CharNode(keys[i], 0, 0, n/totalChars*1000, 0));
    println(keys[i], n, n/totalChars, n/totalChars*1000);
  }
