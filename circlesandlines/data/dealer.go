...
//
// Your job is to complete the exercises as listed in the 
// README file, enhancing this dealer.go and the client
// code in player.go accordingly.
//
...
var request chan scorereport = make(chan scorereport)
// var scorecard = make(map[string]scorereport)
...
// number : int -> int
//
// Converts a card's code (0-51) into its number,
...
// channel 'cin'. 
//
func handleGame(cout chan<- string, cin <-chan string) {
    ...
    request <- scorereport{player:clientname, hands:0, wins:0, losses:0, ties:0}
...
func main() {
    ...
    go func() {
        var scorecards = make(map[string]scorereport)
        ...
        for {
            ...
            case r = <-creport:
                switch r.result {
                case "lost":
                    scorecards[r.player].losses++
                case "won":
                    scorecards[r.player].wins++
                case "tied":
                    scorecards[r.player].ties++
                }
                scorecards[r.player].hands++
            case sr = <-request:
                if scorecards[sr.player].hands != 0 {
                    // Returning player (or, at least, the name given has been
                    // used before); retrieve stored scorecard
                    sr = scorecards[sr.player]

                } else {
                    // New player; store scorecard
                    scorecards[sr.player] = sr
                }
                request <- fmt.Sprintf("[%s's scorecard]\n", sr.player)
                request <- fmt.Sprintf("Hands played: %v\n", sr.hands)
                request <- fmt.Sprintf("Wins: %v\n", sr.wins)
                request <- fmt.Sprintf("Losses: %v\n", sr.losses)
                request <- fmt.Sprintf("Ties: %v\n", sr.ties)
                request <- "\n"
            }
        }
    }()
    ...
    e := cs221.HandleConnections(hostname, port, handleGame, "Dealer")
